...
const IDLE_TIMEOUT_MS = 40000; // 40 seconds idle timeout
const MAX_SCREEN_OBJECTS = 15; // Limit for Sea/Sky to prevent clutter
...
const GameEngine: React.FC<GameEngineProps> = ({ theme, level, onGameOver, onExit }) => {
  ...
  const handleCorrect = (id: string, isBonus = false, clickX?: number, clickY?: number) => {
      resetIdleTimer(); 
      setShowDragHint(false); // Hide hint on success
  ...
  const handleWrong = (letterId: string, clickX?: number, clickY?: number) => {
      ...
      setLives((prev) => {
        ...
        if (newLives <= 0) {
          triggerGameOver(score, 0); // Pass current score (before -5 update actually renders, but close enough for UX)
        } else {
